import React, { Component } from 'react';
import { Link } from 'react-router-dom';
import Navi from '../Navi';
import './Home.css';
import movie1 from '../Movie_poster/movie1.png';
import movie2 from '../Movie_poster/movie2.png';
import movie3 from '../Movie_poster/movie3.png';

function Home() {
       return (
           <>
           <Navi></Navi>
           <br/><br/><br/>
           <p id="logo">Seed</p>
           <p id="logoIntro">Best Movie Review Application</p>
           <p id="joinMsg">Are you not a member yet?</p>
           <Link to="/signup"><div id="joinLink">Join <br/> to <br/> SEED</div></Link>
           <hr id="line"></hr>
           <div id="frame">
               <p id="introTitle">Why SEED?</p>
           <div class="intro" id="intro1">
                <h2>다함께 즐기세요.</h2>
                <p class="intropara">Mobile, Desktop 등 다양한 디바이스에서 리뷰하고 공유하세요.</p>
           </div>

           <div class="intro" id="intro2">
                <h2>글을 저장하세요.</h2>
                <p class="intropara" id="para2">마음에 드는 리뷰가 있다면 간편하게 저장하고, 오프라인에서도 확인하세요.</p>
                <br/>
           </div>

           <div class="intro" id="intro3">
                <h2>전용 프로필을 사용하세요.</h2>
                <p class="intropara" id="para3">리뷰를 작성하고 공유하며 영감을 함께 나눠보세요.</p>
           </div>
           </div>

           
           <div id="posterdiv">
           <p id="recoTitle">Recommened Movie</p>
             <img class= "poster" src={movie1} />
             <img class= "poster" src={movie2} />
             <img class= "poster" src={movie3} />
           </div>
           {/* <p class="introduce">Write Your Review.<br/>See others Review.</p> */}
           </>
       );
}

export default Home;